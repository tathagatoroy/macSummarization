...
config_path = "/home2/tathagato/summarization/MACSUM/naacl/configs/joint_multi_attribute_dpo.yaml"
...
def run_experiment(experiment, gpu_id):
    ...
    if DEBUG:
        command = f"CUDA_VISIBLE_DEVICES={gpu_id} python multi_attribute_joint_training_dpo.py --experiment_name {experiment} --debug > {log_file} 2>&1"
    else:
        command = f"CUDA_VISIBLE_DEVICES={gpu_id} python multi_attribute_joint_training_dpo.py --experiment_name {experiment} > {log_file} 2>&1"
