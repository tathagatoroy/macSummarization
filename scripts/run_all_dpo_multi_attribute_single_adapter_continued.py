...
config_path = "/home2/tathagato/summarization/MACSUM/naacl/configs/multi_attribute_single_adapter_continued_dpo.yaml"
...
def run_experiment(experiment, gpu_id):
    ...
    log_file = f"./logs/multi_attribute_single_adapter_{experiment}_gpu{gpu_id}.log"
    print("Starting experiment", experiment, "on GPU", gpu_id)
    
    if DEBUG:
        command = f"CUDA_VISIBLE_DEVICES={gpu_id} python multi_attribute_single_adapter_continued_dpo.py --experiment_name {experiment} --debug > {log_file} 2>&1"
    else:
        command = f"CUDA_VISIBLE_DEVICES={gpu_id} python multi_attribute_single_adapter_continued_dpo.py --experiment_name {experiment} > {log_file} 2>&1"
