...
config_path = "/home2/tathagato/summarization/MACSUM/naacl/configs/multi_attribute_multi_adapter_sft.yaml"
...
def run_experiment(experiment, gpu_id):
    ...
    if DEBUG:
        command = f"CUDA_VISIBLE_DEVICES={gpu_id} python multi_attribute_multi_adapter_sft.py --experiment_name {experiment} --debug > {log_file} 2>&1"
    else:
        command = f"CUDA_VISIBLE_DEVICES={gpu_id} python multi_attribute_multi_adapter_sft.py --experiment_name {experiment} > {log_file} 2>&1"
