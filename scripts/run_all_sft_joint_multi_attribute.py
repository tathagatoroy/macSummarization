...
def run_experiment(experiment, gpu_id):
    ...
    if DEBUG:
        command = f"CUDA_VISIBLE_DEVICES={gpu_id} python multi_attribute_joint_training_sft.py --experiment_name {experiment} --debug > {log_file} 2>&1"
    else:
        command = f"CUDA_VISIBLE_DEVICES={gpu_id} python multi_attribute_joint_training_sft.py --experiment_name {experiment} > {log_file} 2>&1"
